...
- AI sparkle button now only appears when a supported type is selected in the creation dialog (weapon for items, NPC for actors).
...
- In-place transformation of the native Create Item dialog with prompt textarea.
- Lightweight confirmation dialog with editable name and read-only stats preview.
