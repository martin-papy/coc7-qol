...
- **It only ever tightens visibility.** A roll that already arrives private or blind — because you set a private roll mode, because another module handled it, or because CoC7 fixed it — is left exactly as it is.
...
- When it notices that a hidden combatant's roll already arrived private while your roll mode is public (and this module's own Roll Visibility dropdown was not the cause), it logs a one-line `[coc7-qol]` note in the browser console: CoC7 (or another module) now handles it, and this workaround can be removed.
